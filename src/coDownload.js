...
(() => {
  ...
  function ts(english, params) {
    ...
    Object.keys(params || {}).forEach(prop => { translated.replace('%' + prop, params[prop]); });
    return translated;
  }
        ...
        <div class="idl-locator">
          ...
          <form action='#' >
            ...
            <div class="idl-field first_name">
              <label for="${uniquePrefix}-fn">${ts('First name')}<sup class="red" title="${ts('required')}">*</sup></label>
              <input
                id="${uniquePrefix}-fn"
                name="first_name"
                type="text"
            ...
            <div class="idl-field last_name">
              <label for="${uniquePrefix}-ln">${ts('Last name')}<sup class="red" title="${ts('required')}">*</sup></label>
              <input
                id="${uniquePrefix}-fn"
                name="last_name"
                type="text"
            ...
            <div class="idl-field email">
              <label for="${uniquePrefix}-e1">${ts('Email')}<sup class="red" title="${ts('required')}">*</sup></label>
              <input
                id="${uniquePrefix}-e1"
                name="email"
                type="email"
            ...
            <div class="idl-field email">
              <label for="${uniquePrefix}-e2">${ts('Email again to confirm')}<sup class="red" title="${ts('required')}">*</sup></label>
              <input
                id="${uniquePrefix}-e1"
                name="email2"
                type="email"
            ...
            <div class="idl-field organisation">
              <label for="${uniquePrefix}-o">${ts('Organisation')}</label>
              <input
                id="${uniquePrefix}-o"
                name="organisation"
                type="text"
            ...
            <div class="idl-field question">
              <label for="${uniquePrefix}-q"><span></span><sup class="red" title="${ts('required')}">*</sup></label>
              <textarea
                rows=6 cols=60
                id="${uniquePrefix}-q"
                name="question"
            ...
            <div class="idl-field followup">
              <label for="${uniquePrefix}-fu"><span></span><sup class="red" title="${ts('required')}">*</sup></label>
      ...
      nodes.form.addEventListener('submit', e => {
        ...
        .then((r) => {
          if (!r) {
            ...
            alert(ts("Sorry, there was a problem with the form") + ' :' + r.error);
            cancelSubmission();
          }
          else if (r.token) {
            ...
            setTimeout(() => {
              ...
              .then(r => {
                if (r.success) {
                  ...
                  alert(ts("Sorry, there was a problem with the form") + ' :' + (r.error || 'unknown error'));
                  cancelSubmission();
                  return;
                }
              });
