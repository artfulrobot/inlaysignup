(function(angular, $, _) {
  ...
  angular.module('inlaycontact').config(function($routeProvider) {
      $routeProvider.when('/inlays/contact/:id', {
        controller: 'InlayContact',
        controllerAs: '$ctrl',
        templateUrl: '~/inlaycontact/Inlays.html',
  ...
  angular.module('inlaycontact').controller('InlayContact', function($scope, $route, crmApi4, crmStatus, crmUiHelp, various) {
    // The ts() and hs() functions help load strings for this module.
    var ts = $scope.ts = CRM.ts('inlaycontact');
    var hs = $scope.hs = crmUiHelp({file: 'CRM/inlaycontact/Inlays'}); // See: templates/CRM/inlaycontact/Inlays.hlp
