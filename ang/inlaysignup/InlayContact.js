(function(angular, $, _) {
  ...
  angular.module('inlaysignup').config(function($routeProvider) {
      $routeProvider.when('/inlays/contact/:id', {
        controller: 'InlaysignupInlayContact',
        controllerAs: '$ctrl',
        templateUrl: '~/inlaysignup/InlayContact.html',
  ...
  angular.module('inlaysignup').controller('InlaysignupInlayContact', function($scope, crmApi4, crmStatus, crmUiHelp, various) {
    // The ts() and hs() functions help load strings for this module.
    var ts = $scope.ts = CRM.ts('inlaysignup');
    var hs = $scope.hs = crmUiHelp({file: 'CRM/inlaysignup/InlayContact'}); // See: templates/CRM/inlaysignup/InlayContact.hlp
